...
const totalStock = (input:ProductType[],price:number):number=>{
  return input.filter((item)=>(item.price>price)).reduce((prev,curr)=>(prev+=curr.stock),0)
}
...
/* problem 67 */
const namesOfAllProductsGreatThan = (input:ProductType[],length:number):string[]=>{
  return input.find((item)=>(item.name.length>length))
}
console.log(namesOfAllProductsGreatThan(products,3));